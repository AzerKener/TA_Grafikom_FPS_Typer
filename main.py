...
def play_sfx_shoot():
    ...
    destroy(sfx, delay=2.0)

# ── Lingkungan & Kamera Setup ─────────────────────────────────────────────────
ground = Entity(
...
player.gravity           = 0
player.speed             = 0
player.mouse_sensitivity = Vec2(0, 0) # Kunci trackpad & mouse manual
...
home_subtitle = Text(
    ...
    origin=(0, 0), position=(0, 0.08), scale=1.8, color=color.light_gray, z=0   
)
...
# ── GAME OVER SCREEN ──────────────────────────────────────────────────────────
gameover_bg = Entity(parent=camera.ui, model='quad', color=color.rgba(0, 0, 0, 200), scale=(2, 2), z=1)
gameover_bg.visible = False
gameover_title = Text(text='GAME OVER', parent=camera.ui, origin=(0, 0), position=(0, 0.18), scale=6, color=color.red, z=0)
gameover_title.visible = False
gameover_score = Text(text='', parent=camera.ui, origin=(0, 0), position=(0, 0.04), scale=2.2, color=color.white, z=0)
...
# ── SETTINGS SCREEN ───────────────────────────────────────────────────────────
settings_bg = Entity(parent=camera.ui, model='quad', color=color.rgba(0, 0, 0, 230), scale=(2, 2), z=1)
settings_bg.visible = False
settings_title = Text(text='SETTING', parent=camera.ui, origin=(0, 0), position=(0, 0.3), scale=4, color=color.cyan, z=0)
...
class VolumeSlider(Entity):
    def __init__(self, label_text, pos, initial_value, on_change):
        ...
        self.lbl = Text(text=label_text, parent=camera.ui, position=(pos[0] - 0.22, pos[1] + 0.005), origin=(1, 0), scale=2, color=color.white, z=0)
        self.track = Entity(parent=camera.ui, model='quad', color=color.rgba(80, 80, 80, 200), scale=(self.track_width, 0.018), position=pos, z=0)
        self.fill = Entity(parent=camera.ui, model='quad', color=color.cyan, scale=(self.track_width * initial_value, 0.018), position=(pos[0] - self.track_width/2 * (1 - initial_value), pos[1]), z=0)
        self.handle = Entity(parent=camera.ui, model='circle', color=color.white, scale=(0.022, 0.022), position=(pos[0] - self.track_width/2 + self.track_width * initial_value, pos[1]), z=0)
        self.pct_label = Text(text=f'{int(initial_value*100)}%', parent=camera.ui, position=(pos[0] + self.track_width/2 + 0.04, pos[1] + 0.005), origin=(0, 0), scale=1.8, color=color.white, z=0)
        self._base_x = pos[0]

    def set_visible(self, v):
        self.lbl.visible = v; self.track.visible = v; self.fill.visible = v; self.handle.visible = v; self.pct_label.visible = v
    ...
    def input(self, key):
        if not self.track.visible: return
        if key == 'left mouse down':
            hx, hy = self.handle.x, self.handle.y
            if abs(mouse.x - hx) < 0.04 and abs(mouse.y - hy) < 0.04: self.dragging = True
        if key == 'left mouse up': self.dragging = False

    def update(self):
        if self.dragging and self.track.visible: self.update_from_mouse()

def _set_vol_musik(v):
    global vol_musik; vol_musik = v; bg_music.volume = v
def _set_vol_sfx(v):
    global vol_sfx; vol_sfx = v
...
def trigger_typo_indicator():
    global _typo_active; _typo_active = True; ui_target_box.color = color.red
    def restore_typo():
        global _typo_active; ui_target_box.color = color.yellow; _typo_active = False
    invoke(restore_typo, delay=0.4)

def show_typo_on_label(entity_with_label, word, done_count, wrong_letter):
    done, wrong, rest = word[:done_count], wrong_letter, word[done_count + 1:]
    entity_with_label.text_label.text = f"<green>{done}</green><red>{wrong}</red>{rest}"
...
def show_typo_on_boss_label(word, done_count, wrong_letter):
    ...
    if boss_entity and boss_entity.alive:
        boss_entity.text_label.text = f"<green>{done}</green><red>{wrong}</red>{rest}"
        invoke(lambda: update_boss_ui(), delay=0.35)

# ─────────────────────────────────────────────────────────────────────────────
# SPAWN HELPER (FIX: Menggunakan Jarak Pythagoras Mutlak Sumbu X & Z)
# ─────────────────────────────────────────────────────────────────────────────
def find_safe_spawn_position():
    for _ in range(SPAWN_MAX_TRIES):
        ...
        if math.hypot(candidate.x - player.x, candidate.z - player.z) < SPAWN_MIN_DIST:
            continue

        too_close = any(
            math.hypot(candidate.x - e.x, candidate.z - e.z) < SPAWN_MIN_SEP
            for e in enemies_list
        )
        if not too_close: return candidate

    angle = random.uniform(0, 2 * math.pi)
    return Vec3(player.x + math.sin(angle) * SPAWN_MAX_DIST, 2, player.z + math.cos(angle) * SPAWN_MAX_DIST)

# ─────────────────────────────────────────────────────────────────────────────
# ENEMY (Normal) (FIX: Konflik Perputaran Karakter 2D & Pergerakan Selesai)
# ─────────────────────────────────────────────────────────────────────────────
class Enemy(Entity):
    def __init__(self, word):
        ...
        super().__init__(
            ...
            position=spawn_pos, scale=(3.5, 4.2), billboard=True
        )
        self.word     = word
        self.speed    = random.uniform(0.4, 0.7) + (current_wave * 0.04)
        self.alive    = True
        self.can_move = False

        self.text_label = Text(
            text=self.word, parent=self, position=(0, 0.7, 0),
            scale=4, origin=(0, 0), background=True,
...
class Boss(Entity):
    def __init__(self, paragraph_str):
        ...
        self.text_label = Text(
            text='BOSS', parent=self, position=(0, 0.8, 0),
            scale=3, origin=(0, 0), background=True,
            color=color.red, billboard=True
        )
        minimap_dots[self] = Entity(parent=minimap_bg, model='circle', color=color.magenta, scale=(0.07, 0.07))
...
# ─────────────────────────────────────────────────────────────────────────────
# HP & ACTION MANAGEMENT
# ─────────────────────────────────────────────────────────────────────────────
def remove_enemy_completely(enemy_obj):
    if not enemy_obj.alive: return
    enemy_obj.alive = False
    if enemy_obj in enemies_list: enemies_list.remove(enemy_obj)
...
def shoot_bullet(target_entity):
    if not target_entity or not target_entity.alive: return
    ...
    player_weapon.x = 0.38; player_weapon.y = -0.35
    player_weapon.animate_x(0.4, duration=0.05); player_weapon.animate_y(-0.3, duration=0.05)

    spawn_pos   = player.position + Vec3(0, 1.2, 0) + player.forward * 1.5
    bullet      = Entity(model='quad', texture='peluru.png', position=spawn_pos, scale=(0.3, 0.3), billboard=True, color=color.white)
...
# ─────────────────────────────────────────────────────────────────────────────
def show_boss_ui():
    boss_hp_bar_bg.visible = True; boss_hp_bar_fill.visible = True; boss_hp_label.visible = True; ui_boss_paragraph.visible = True
    ui_target_box.position = (0, 0.35); ui_typing_progress.position = (0, 0.28)
    update_boss_ui()

def hide_boss_ui():
    boss_hp_bar_bg.visible = False; boss_hp_bar_fill.visible = False; boss_hp_label.visible = False; ui_boss_paragraph.visible = False
    ui_target_box.position = (0, -0.3); ui_typing_progress.position = (0, -0.05)
    ui_target_box.text = ''; ui_typing_progress.text = ''

def update_boss_ui():
    global boss_entity, boss_words, boss_word_index, boss_letter_index
    if not boss_entity or not boss_entity.alive: return

    total, done_n = boss_entity.total_words, boss_word_index
    ratio = (total - done_n) / total if total > 0 else 0

    bar_full = 0.7; fill_w = bar_full * ratio; fill_x = -bar_full / 2 + fill_w / 2
    boss_hp_bar_fill.scale_x = fill_w; boss_hp_bar_fill.x = fill_x
    boss_hp_label.text = f'BOSS HP: {total - done_n}/{total} KATA'

    parts = []
    for i, w in enumerate(boss_words):
        if i < boss_word_index: parts.append(f"<green>{w}</green>")
        elif i == boss_word_index:
            done_letters, remain_letters = w[:boss_letter_index], w[boss_letter_index:]
...
# ─────────────────────────────────────────────────────────────────────────────
# TARGET LOCKING (Instant Auto-Lock Terdekat)
# ─────────────────────────────────────────────────────────────────────────────
def auto_lock_nearest_enemy():
    ...
    if enemies_list and not game_over and not is_boss_wave:
        # Kunci musuh berdasarkan jarak absolut Pythagoras terdekat murni sumbu X & Z
        active_target = min(enemies_list, key=lambda e: math.hypot(e.x - player.x, e.z - player.z))
        current_input_index = 0
        update_text_visual()
    else:
        active_target = None
...
def request_lock():
    ...
    if not _lock_requested and not is_boss_wave:
        _lock_requested = True; invoke(auto_lock_nearest_enemy, delay=0)

def reset_targeting():
    global active_target, current_input_index
    if active_target and active_target in enemies_list: active_target.text_label.text = active_target.word
    active_target = None; current_input_index = 0
    ui_target_box.text = ''; ui_typing_progress.text = ''
...
def update():
    ...
    # Logika kamera Auto-Lock jika masuk Boss Wave
    if is_boss_wave:
        if boss_entity and boss_entity.alive:
    ...
    # Logika kamera Auto-Lock jika Normal Wave
    ...
    if is_boss_wave and boss_entity and boss_entity.alive:
        ...
        if key == 'space':
            if boss_letter_index == len(current_word):
                ...
                boss_word_index  += 1; boss_letter_index = 0
                if boss_word_index >= boss_total_words: defeat_boss()
                else: update_boss_ui()
            else: trigger_typo_indicator()
            return
        ...
        pressed, expected = key.upper(), current_word[boss_letter_index]
        if pressed == expected:
            boss_letter_index += 1; shoot_bullet(boss_entity); update_boss_ui()
        else:
            show_typo_on_boss_label(current_word, boss_letter_index, expected)
            boss_letter_index = 0; trigger_typo_indicator()
        return

    # ── INPUT ENGINE: NORMAL WAVE ────────────────────────────────────────────
    if key == 'space' or len(key) != 1: return
    ...
    if active_target and active_target in enemies_list and active_target.alive:
        ...
        if pressed_letter == target_word[current_input_index]:
            ...
            if current_input_index == len(target_word):
                finished_word = active_target.word   
            ...
            # JIKA TYPO: Kunci target tetap ada, progress ketikan diulang ke 0
...
def update_text_visual():
    if active_target and active_target.alive:
        done, remain = active_target.word[:current_input_index], active_target.word[current_input_index:]
        if not _typo_active: active_target.text_label.text = f"<green>{done}</green>{remain}"
        ui_target_box.text      = f"TARGET: {active_target.word}"
        ui_typing_progress.text = done

# ─────────────────────────────────────────────────────────────────────────────
# WAVE & THEME PROGRESSION MANAGEMENT
# ─────────────────────────────────────────────────────────────────────────────
def defeat_boss():
    global boss_entity, is_boss_wave, current_theme_index
    if boss_entity:
        remove_enemy_completely(boss_entity)
        boss_entity = None
    is_boss_wave = False
...
def spawn_boss():
    ...
    ui_announcement.text = ''; ui_stats.text = f'HP: {player_hp} | WAVE: {current_wave} [BOSS]'
    show_boss_ui()
    invoke(lambda: setattr(boss_entity, 'can_move', True), delay=GRACE_PERIOD)

def activate_all_enemies():
    global wave_grace_active; wave_grace_active = False
    for e in enemies_list:
        if e.alive: e.can_move = True
    ui_announcement.text = ''
...
def spawn_wave():
    ...
    if is_boss_wave:
        ...
        for _ in range(num_enemies):
            if current_wave <= 2: word = random.choice(IT_WORDS_EASY)
            elif current_wave == 4: word = random.choice(IT_WORDS_EASY + IT_WORDS_MEDIUM)
            else: word = random.choice(IT_WORDS_MEDIUM + IT_WORDS_HARD)
...
def check_wave_clear():
    global current_wave
    if is_boss_wave and boss_entity is not None: return
